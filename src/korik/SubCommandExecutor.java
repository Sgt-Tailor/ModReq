...
public abstract class SubCommandExecutor implements CommandExecutor {
	 ...
	 * All credits for Korikisulda for making this class :D
	 */
	...
	public void onSubCommand(CommandSender sender,String[] arguments,String commandName){
		...
		try {
			for(Method m : this.getClass().getMethods()) {
				if(m.getName().equalsIgnoreCase(commandName) || (isInteger(commandName) && m.getName().equals("Integer"))) {
	...
	private boolean isInteger(String string) {
		try {
			@SuppressWarnings("unused")// it is used to check if it is a integer or not
